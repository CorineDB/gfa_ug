...
const createData = async () => {
  ...
  await FondsService.create(payload)
    .then(() => {
      isLoading.value = false;
      getDatas();
      resetForm();
      toast.success("Fond créer.");
    })
    .catch((e) => {
      isLoading.value = false;
...
const updateData = async () => {
  isLoading.value = true;
  await FondsService.update(idSelect.value, payload)
    .then(() => {
      isLoading.value = false;
      getDatas();
      resetForm();
      toast.success("Fond modifiée.");
    })
    .catch((e) => {
      isLoading.value = false;
      toast.error("Vérifier les informations et ressayer.");
...
const deleteData = async () => {
  isLoading.value = true;
  await FondsService.destroy(idSelect.value)
    .then(() => {
      deleteModalPreview.value = false;
      isLoading.value = false;
      toast.success("Fond supprimée");
      getDatas();
    })
    .catch((e) => {
      isLoading.value = false;
...
const handleEdit = (params) => {
  ...
  payload.nom_du_fond = params.nom_du_fond;
  payload.fondDisponible = params.fondDisponible;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Fonds</h2>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="flex flex-wrap items-center justify-between col-span-12 mt-2 intro-y sm:flex-nowrap">
      <div class="w-full mt-3 sm:w-auto sm:mt-0 sm:ml-auto md:ml-0">
        <div class="relative w-56 text-slate-500">
          <input type="text" id="search_fonds" name="search_fonds" @input="applyFilter" v-model="searchValue" class="w-56 pr-10 form-control box" placeholder="Recherche..." />
          <SearchIcon class="absolute inset-y-0 right-0 w-4 h-4 my-auto mr-3" />
        </div>
      </div>
      <div class="flex">
        <button class="mr-2 shadow-md btn btn-primary" @click="openCreateModal"><PlusIcon class="w-4 h-4 mr-3" />Ajouter un Fond</button>
      </div>
    </div>
  </div>
  ...
  <Modal backdrop="static" :show="showModalCreate" @hidden="showModalCreate = false">
    <ModalHeader>
      <h2 class="mr-auto text-base font-medium">{{ mode }} un Fond</h2>
    </ModalHeader>
  ...
  <Modal :show="deleteModalPreview" @hidden="deleteModalPreview = false">
    <ModalBody class="p-0">
      <div class="p-5 text-center">
        ...
        <div class="mt-2 text-slate-500">Supprimer cet Fond?</div>
      </div>
