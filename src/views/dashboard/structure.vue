  ...
  <div class="py-4">
    ...
    <Modal :show="deleteModalPreview" @hidden="deleteModalPreview = false">
      <ModalBody class="p-0">
        ...
        <div class="px-5 pb-8 text-center">
          ...
          <button type="button" @click="deleteStructure" class="btn btn-danger w-24">Supprimer</button>
        </div>
      </ModalBody>
    </Modal>
    <!-- END: Modal Content -->
    ...
    <Modal :show="showModal" @hidden="close">
      <ModalBody class="p-10">
        <form v-if="!isUpdate" key="ajouter" @submit.prevent="storeStructure">
          <div class="my-2">
            <label for="regular-form-1" class="form-label">Nom</label>
            <input id="regular-form-1" type="text" required v-model="formData.nom" class="form-control" placeholder="libellé du structure" />
          </div>
          <div class="my-2">
          ...
          <div class="my-2">
            <label for="regular-form-1" class="form-label"> Point Focal </label>
            <input id="regular-form-1" type="text" required v-model="formData.pointFocal" class="form-control" placeholder="Description" />
          </div>
        ...
        <form v-else key="modifier" @submit.prevent="updateStructure">
          <div class="my-2">
            <label for="regular-form-1" class="form-label">Nom</label>
    ...
    <div class="flex justify-between">
      <button @click="addStructure" class="btn btn-primary flex space-x-2 items-center">
        <PlusSquareIcon />
      ...
      <table class="table mt-5">
        <thead class="table-light">
          <tr>
            ...
            <th class="whitespace-nowrap">Point Focal</th>
            <th class="whitespace-nowrap">Créer le</th>
              ...
              <Dropdown class="inline-block" placement="top-end">
                ...
                <DropdownMenu class="w-40">
                  <DropdownContent>
                    ...
                    <Tippy tag="a" href="javascript:;" class="tooltip inline-block my-2" content="cliquez pour voir les stats de ce indicateur">
                      <span class="text-black cursor-pointer flex justify-start items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trending-up mr-2">
                          <polyline points="23 6 13.5 15.5 8.5 10.5 1 18"></polyline>
                          <polyline points="17 6 23 6 23 12"></polyline></svg
                        >Voir Stats</span
                      >
                    </Tippy>
                    <Tippy tag="a" href="javascript:;" class="tooltip inline-block my-2" content="cliquez pour exporter les stats de ce indicateur">
                      <span class="text-black cursor-pointer flex justify-start items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-upload mr-2">
              ...
              <div class="tooltip-content">
                <TippyContent :to="'custom-tooltip-content' + index">
                  <div :id="'custom-content-tooltip' + index" class="relative">
                    <div class="my-1">Date de création : {{ data.created_at }}</div>
                    <div class="my-1">Date de mise à jour : {{ data.updated_at }}</div>
                  </div>
                </TippyContent>
              </div>
            </td>
...
const resultQuery = computed(() => {
  if (search.value) {
    return structures.value.filter((item) => {
      return (
        search.value
          .toLowerCase()
          .split(" ")
    ...
    return structures.value.slice(startIndex, endIndex);
  }
});
...
function totalPages() {
  return Math.ceil(structures.value.length / itemsPerPage.value);
}
...
const addStructure = function () {
  showModal.value = true;
  isUpdate.value = false;
};

const storeStructure = function () {
  if (chargement.value == false) {
    chargement.value = true;
    StructureService.create(formData)
      .then((data) => {
        message.type = "success";
        message.message = "Nouveaux structure créee";
...
const deleteStructure = function () {
  ...
  structures.value.splice(structures.value.indexOf(deleteData.index), 1);
  StructureService.destroy(deleteData.id)
    .then((data) => {
      message.type = "success";
...
const updateStructure = function () {
  if (chargement.value == false) {
    chargement.value = true;
    const formData = {
      nom: saveUpdate.nom,
    ...
    StructureService.update(saveUpdate.id, formData)
      .then((data) => {
        chargement.value = false;
        message.type = "success";
