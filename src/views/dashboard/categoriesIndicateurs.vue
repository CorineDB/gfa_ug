...
const createData = async () => {
  ...
  await CategoriesService.create(payload)
    .then(() => {
      ...
      getDatas();
      resetForm();
      toast.success("Catégories d'Indicateurs créer.");
    })
    .catch((e) => {
      isLoading.value = false;
      console.error(e);
...
const updateData = async () => {
  isLoading.value = true;
  await CategoriesService.update(idSelect.value, payload)
    .then(() => {
      isLoading.value = false;
      getDatas();
      resetForm();
      toast.success("Catégories d'Indicateurs modifiée.");
    })
    .catch((e) => {
      isLoading.value = false;
      console.error(e);
...
const deleteData = async () => {
  ...
  await CategoriesService.destroy(idSelect.value)
    .then(() => {
      ...
      isLoading.value = false;
      toast.success("Catégories d'Indicateurs supprimée");
      getDatas();
    })
    .catch((e) => {
      isLoading.value = false;
...
<template>
  <h2 class="mt-10 text-lg font-medium intro-y">Catégories d'Indicateurs</h2>
  <div class="grid grid-cols-12 gap-6 mt-5">
    <div class="flex flex-wrap items-center justify-between col-span-12 mt-2 intro-y sm:flex-nowrap">
      ...
      <div class="flex">
        <button class="mr-2 shadow-md btn btn-primary" @click="openCreateModal"><PlusIcon class="w-4 h-4 mr-3" />Ajouter une Catégories d'Indicateurs</button>
      </div>
    </div>
  </div>
  ...
  <Modal backdrop="static" :show="showModalCreate" @hidden="showModalCreate = false">
    <ModalHeader>
      <h2 class="mr-auto text-base font-medium">{{ mode }} une Catégories d'Indicateurs</h2>
    </ModalHeader>
  ...
  <Modal :show="deleteModalPreview" @hidden="deleteModalPreview = false">
    <ModalBody class="p-0">
      <div class="p-5 text-center">
        ...
        <div class="mt-2 text-slate-500">Supprimer cette Catégories d'Indicateurs?</div>
      </div>
