...
<template>
  ...
  <div v-if="!isLoadingData" class="grid grid-cols-12 gap-6 mt-5">
    ...
    <div v-for="(item, index) in sousComposants" :key="index" class="col-span-12 intro-y md:col-span-6 lg:col-span-4">
      <div v-if="verifyPermission('voir-un-output')" class="p-5 box">
        <div class="flex items-start pt-5 _px-5">
          ...
          <Dropdown class="absolute top-0 right-0 mt-3 mr-5">
            ...
            <DropdownMenu class="w-40">
              <DropdownContent>
                <DropdownItem v-if="verifyPermission('modifier-un-output')" @click="modifierSousComposante(item)"> <Edit2Icon class="w-4 h-4 mr-2" /> Modifier </DropdownItem>
                <DropdownItem v-if="verifyPermission('supprimer-un-output')" @click="supprimerComposant(item)"> <TrashIcon class="w-4 h-4 mr-2" /> Supprimer </DropdownItem>
              </DropdownContent>
            </DropdownMenu>
          </Dropdown>
        </div>
