  ...
  <div class="py-4">
    ...
    <Modal :show="deleteModalPreview" @hidden="deleteModalPreview = false">
      <ModalBody class="p-0">
        ...
        <div class="px-5 pb-8 text-center">
          ...
          <button type="button" @click="deleteChoix" class="btn btn-danger w-24">Supprimer</button>
        </div>
      </ModalBody>
    </Modal>
    <!-- END: Modal Content -->
    ...
    <Modal :show="showModal" @hidden="close">
      <ModalBody class="p-10">
        <form v-if="!isUpdate" key="ajouter" @submit.prevent="storeChoix">
          <div class="my-2">
            <label for="regular-form-1" class="form-label">Nom</label>
            <input id="regular-form-1" type="text" required v-model="formData.nom" class="form-control" placeholder="libellé du Choix" />
          </div>
          <div class="my-2">
          ...
          <div class="my-2">
            ...
            <input id="regular-form-1" type="number" step="0.01" required v-model="formData.poid" class="form-control" placeholder="Description" />
          </div>
        ...
        <form v-else key="modifier" @submit.prevent="updateChoix">
          <div class="my-2">
            <label for="regular-form-1" class="form-label">Nom</label>
    ...
    <div class="flex justify-between">
      <button @click="addChoix" class="btn btn-primary flex space-x-2 items-center">
        <PlusSquareIcon />
...
const resultQuery = computed(() => {
  if (search.value) {
    return Choix.value.filter((item) => {
      return (
        search.value
          .toLowerCase()
          .split(" ")
    ...
    return Choix.value.slice(startIndex, endIndex);
  }
});
...
function totalPages() {
  return Math.ceil(Choix.value.length / itemsPerPage.value);
}
...
const addChoix = function () {
  showModal.value = true;
  isUpdate.value = false;
};

const storeChoix = function () {
  if (chargement.value == false) {
    chargement.value = true;
    ChoixService.create(formData)
      .then((data) => {
        message.type = "success";
        message.message = "Nouveaux Choix créee";
...
const deleteChoix = function () {
  deleteModalPreview.value = false;
  Choix.value.splice(Choix.value.indexOf(deleteData.index), 1);
  ChoixService.destroy(deleteData.id)
    .then((data) => {
      message.type = "success";
...
const updateChoix = function () {
  if (chargement.value == false) {
    ...
    ChoixService.update(saveUpdate.id, formData)
      .then((data) => {
        chargement.value = false;
        message.type = "success";
