<template>
  <dashboard>
    <div v-if="composanteVisible" class="w-full">
 ...
 mb-2" >
      ...
      <div class="">
        <composante-current :valueProfondeur="{ codePta: $route.params.code, nom: $route.params.nom }" v-if="currentPage">
        </composante-current>
    ...
    <div v-if="currentRole ==='bailleur'" class="w-full">
      ...
      <div class="grid grid-cols-1 my-4 lg:grid-cols-3 md:grid-cols-2 sm:gap-5">
        <div v-for="(composante, index) in composantes" :key="index">
          <card-composante :libelle="composante.nom" :code="composante.code" :statut="getStatus(composante.statut)"
            :montantEPM="composante.pret" :position="composante.position" :poids="composante.poids"
            :bailleur="composante.sigle" @validation="validation(composante)" :isDelete="false" :isUpdate="false">
          </card-composante>
        </div>
