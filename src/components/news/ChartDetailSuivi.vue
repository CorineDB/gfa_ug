...
const formatChartData = () => {
  const labels = props.data.map((item) => `T${item.trimestre} - ${item.valeurCible.annee}`);
  const realisedValues = props.data.map((item) => item.valeurRealise.moy);
  const targetValues = props.data.map((item) => item.valeurCible.valeurCible.moy);
