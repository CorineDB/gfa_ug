...
const submitData = async () => {
  ...
  const action = isCreate.value ? UniteeDeMesureService.create(payload) : UniteeDeMesureService.update(idSelect.value, payload);
  try {
    await action;
    toast.success(`Unité de mesure ${isCreate.value ? "créee" : "modifiée"} avec succès.`);
    getDatas();
    resetForm();
  } catch (e) {
    ...
    <Modal :show="deleteModalPreview" @hidden="closeDeleteModal">
      <ModalBody class="p-0">
        <div class="p-5 text-center">
          ...
          <div class="mt-2 text-slate-500">Supprimer l'unité de mesure?</div>
        </div>
