          ...
          <div v-if="principe.nom != ''" class="flex items-center gap-1 space-x-1 transition-all container-buttons">
            ...
            <div v-if="canEditPrincipe">
              <input type="number" min="1" step="1" name="position" :value="principe.position"
                @keyup.enter="updateTemporyPrincipe(principe.id, $event.target.value)" class="w-2/5 form-control" />
            </div>
            <div v-else>
        ...
        <div class="text-wrap">
          {{ item.indicateur.id ? item.indicateur.position + " - " +
            ...
            <div v-if='item.indicateur.nom != ""'
              class="flex items-center gap-1 space-x-1 transition-all container-buttons">

              <div v-if="canEditQuestion[item.key]">
                <input type="number" min="1" step="1" name="position" :value="item.indicateur.position"
                  @keyup.enter="updateTemporyQuestion(item.key, $event.target.value)" class="w-2/5 form-control" />
              </div>
              <div v-else>
