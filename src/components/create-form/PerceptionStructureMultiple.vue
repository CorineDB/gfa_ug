...
<template>
  <div class="flex flex-col-reverse items-start w-full gap-2">
    ...
    <div v-for="item in indicateurArray" :key="item.id" :class="[item.indicateur.nom ? 'text-black border-yellow-200 border-l-2 bg-white rounded shadow' : 'border-dashed border-gray-400 text-gray-600 border-2']" class="w-full px-2 py-2.5 text-base truncate font-medium transition-all">{{ item.indicateur.nom || item.indicateur.nom?.length < 0 ? item.indicateur.nom : "Question opérationnelle" }}</div>
