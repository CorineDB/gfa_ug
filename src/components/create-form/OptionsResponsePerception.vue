...
//import FormulairePerception from "@/services/modules/formperception.service";
import FormulairePerception from "@/services/modules/enquetes_de_gouvernance/formPerception.service";
...
const payload = reactive({ libelle: "", type: "perception", description: "" });
const idSelect = ref("");
const idChecked = ref([]);
...
const getDatas = async () => {
  try {
    ...
    const { data } = await OptionReponse.perception();
...
const getOneForm = async () => {
  try {
    const { data } = await FormulairePerception.getOne(props.idForm);
